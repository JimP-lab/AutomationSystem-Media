## AutomationSystem-Media ## 

AutomationSystem-Media is an automatic system that posts to social media every hour. It uses Switchboard to create image posts, SocialBee to upload and manage posts, and the Meta Graph API to fetch the latest posts. The system matches the correct links with each post and uses code to extract content from RSS feeds, making it possible to manage and post to multiple websites at the same time using Business Manager. This project was created with help from the Social AI Automation Assistant.

 ## Workflow ##  
![Platform-DK - draft](https://github.com/user-attachments/assets/3025b393-5c14-463b-8731-2545cb18d065)

 ## Websites ## 
- [AllYou](https://www.allyou.gr) 
- [Avopolis](https://www.avopolis.gr) 
- [Platform](https://www.platform.gr) 
- [Childit](https://childit.gr) 
- [Tetrapodo](https://www.tetrapodo.gr) 

 ## Technologies and Tools ## 
- [Zapier](https://zapier.com)
- [Switchboard](https://www.switchboard.ai)
- [Social Bee](https://socialbee.com)

## Additional Resources ##   
- [Business Manager](https://business.facebook.com/business)
- [Graph Api](https://developers.facebook.com/docs/graph-api)
- [Postman](https://www.postman.com)
- [How to create image templates using switchboard canvas](https://youtu.be/1dwn6ZKtoJc?si=G0oiOgaJhSpMqVUx) 
- [How to run api calls using Switchboard Canvas](https://youtu.be/mxVGxCaEu6k?si=TJd041RA0HRjADxM)
- [how to create a post in social bee inside zapier](https://youtu.be/xc39xAFmDXw?is=DMtt8JcyLmbhIo14)
- [Social ai automation assistant](https://chatgpt.com/g/g-67af3d18d6f88191867abe3aac0bd223-social-ai-automation-assistant)

## Business Manager Meta Dashboard ## 
<img width="900" height="908" alt="Screenshot 2025-10-11 at 21-57-06 Meta Business Suite" src="https://github.com/user-attachments/assets/e0ef73dc-004b-41c7-b43f-65df4a5a48d1"/>
